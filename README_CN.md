...
**当前版本：** v0.8.0 — 画布、四个框架适配器和 Langfuse 可观测性已完成。线束阶段 P0–P4 已完成 · 106 个测试通过 · P5（执行与验证）已解锁。
...
这不仅仅是程度上的差异 — 而是架构上的根本差异。工作流将提示从一个节点路由到另一个节点。线束管理智能体*相信*什么、它*被允许*做什么、如何捕获自身错误，以及下次学到什么。Its Harness 正在构建这一完整架构：今天在画布上绘制，在任何框架上运行，并随着每个阶段的落地获得完整控制层。

### 今日现状 vs 完整目标

| | 今日 — v0.8.0 | 目标 — 完整线束 |
|---|---|---|
| 画布 | 14 种节点类型 | 24 种节点类型（P10 新增 10 种线束节点）|
| 框架 | LangGraph · CrewAI · Mastra · MAF | 全部四个 + 线束层适配器 |
| 可观测性 | 所有 4 个运行时的 Langfuse 追踪 | 线束跨度：世界模型、控制状态、验证、恢复 |
| 推理 | 证据存储 · 工具可靠性包络 · VOI · 假设系统 *（P1 ✓）* | 与执行循环完整集成 |
| 世界模型 | 信念依赖图 · 类型化矛盾 · 陈旧性扫描 *（P2 ✓）* | 与线束跨度完整集成 |
| 控制 | 5 层解析器 · NORMAL / CAUTIOUS / BLOCKED · 106 个测试 *（P3 ✓）* | 跨所有框架的线束控制跨度 |
| 规划 | 任务图（6 状态）· 并行并发 · `conflict_probability_cache` *（P4 ✓）* | 完整适配器集成 |
| 验证 | 无 | 9 层 · 对抗性审查员通过 · 预执行门控 |
| 恢复 | 无 | 6 种命名策略 · 类型化故障库 · 局部/全局重规划 |
| 内存 | 无 | 预算跟踪 · 日志 · 压缩风险 |
| 学习 | 无 | 经验存储 — 跨运行结构复用 |
...
| 阶段 | 名称 | 重点 |
|---|---|---|
| **P0** ✓ | 基础与状态架构 | FlowSpec 扩展、线束状态模型、`generation_id`、`completeness_flags`、健康向量 |
| **P1** ✓ | 证据与推理 | 证据存储、工具可靠性包络、VOI 估计、假设系统 |
| **P2** ✓ | 世界模型与矛盾 | 信念依赖图、类型化矛盾检测、陈旧性扫描 |
| **P3** ✓ | 诊断与控制状态 | 健康向量计算、五层 `resolve_control_state()`、死锁检测 |
| **P4** ✓ | 规划与任务图 | 任务图（6 状态）、并行写域冲突检测、`conflict_probability_cache` |
| **P5** *(进行中)* | 执行与验证 | 预执行审查门控、9 层验证、对抗性通过、可逆性策略 |
| **P6** | 恢复与内存 | 六种命名恢复策略、类型化故障库、上下文压缩、日志保留 |
| **P7** | 调用者状态与上报 | 主动循环中调用者更新、`cannot_make_progress()`、约束变化传播 |
| **P8** | 经验存储 | 跨运行结构复用 — 分解、工作流、恢复序列、策略权重 |
| **P9** | 审查员通过与输出合同 | 三镜头审查员、对抗性先验、完整输出合同验证 |
| **P10** | 画布集成 | 10 种新线束画布节点类型、诊断健康仪表板、更新的框架适配器 |
| **P11** | E2E 集成与测试 | 覆盖所有四个框架上所有 10 个架构不变量的端到端测试 |

完整架构参考（伪代码、节点定义、层表、状态模型和范围）请参阅 [plan/full_harness_architecture.html](https://github.com/3IVIS/itsharness/blob/main/plan/full_harness_architecture.html)。
...
| [plan/full_harness_architecture.html](https://github.com/3IVIS/itsharness/blob/main/plan/full_harness_architecture.html) | 完整架构 — 伪代码、所有 22 个节点、所有 11 层、状态模型、范围 |
| [plan/harness_implementation_plan.html](https://github.com/3IVIS/itsharness/blob/main/plan/harness_implementation_plan.html) | 完整 11 阶段实施计划 |
...
- **查看实施计划** → [plan/harness_implementation_plan.html](https://github.com/3IVIS/itsharness/blob/main/plan/harness_implementation_plan.html)
