...
async def _build_org_out(org: Org, db: AsyncSession) -> OrgOut:
    ...
    members = [MemberOut(user_id=str(u.id), email=u.email, role=m.role) for m, u in memberships]
    return OrgOut(
        id=str(org.id),
        name=org.name,
...
async def list_orgs(user: AuthDep, db: DbDep):
    ...
    return [OrgSummary(id=str(org.id), name=org.name, role=m.role) for m, org in rows]
...
@limiter.limit("20/minute")
async def update_org(request: Request, org_id: str, req: OrgUpdate, user: AuthDep, db: DbDep):
...
@limiter.limit("20/minute")
async def delete_org(request: Request, org_id: str, user: AuthDep, db: DbDep):
...
async def list_members(request: Request, org_id: str, user: AuthDep, db: DbDep):
    ...
    return [MemberOut(user_id=str(u.id), email=u.email, role=m2.role) for m2, u in rows]


@router.post("/{org_id}/members", response_model=MemberOut, status_code=201)
@limiter.limit("20/minute")
async def invite_member(request: Request, org_id: str, req: MemberInvite, user: AuthDep, db: DbDep):
    ...
    if existing:
        ...
        return JSONResponse(
            status_code=200,
            content=MemberOut(user_id=str(invitee.id), email=invitee.email, role=existing.role).model_dump(),
        )
...
async def change_member_role(
    request: Request, org_id: str, member_id: str, req: MemberRoleUpdate, user: AuthDep, db: DbDep
):
...
@limiter.limit("20/minute")
async def remove_member(request: Request, org_id: str, member_id: str, user: AuthDep, db: DbDep):
