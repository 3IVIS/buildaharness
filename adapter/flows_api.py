...
async def _get_flow(flow_id: str, user: User, db: AsyncSession,
                    org: Org | None = None) -> Flow:
...
async def delete_flow(
    request: Request,
    flow_id: str, user: AuthDep, db: DbDep, org: OrgDep):
    flow = await _get_flow(flow_id, user, db, org)
...
async def restore_version(
    request: Request,
    flow_id: str, version_id: str, user: AuthDep, db: DbDep, org: OrgDep):
    flow = await _get_flow(flow_id, user, db, org)
    ver  = (await db.execute(
        select(FlowVersion).where(FlowVersion.flow_id == flow_id,
                                  FlowVersion.id      == version_id)
