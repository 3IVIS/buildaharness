...
async def list_components(
    ...
    stmt = (
        stmt
        .order_by(CommunityComponent.verified.desc(), CommunityComponent.install_count.desc())
        .limit(limit)
        .offset(offset)
    )
...
async def get_component(
    ...
    row = (await db.execute(
        select(CommunityComponent).where(CommunityComponent.slug == slug)
    )).scalar_one_or_none()
    if not row:
...
async def publish_component(
    ...
    existing = (await db.execute(
        select(CommunityComponent).where(CommunityComponent.slug == req.slug)
    )).scalar_one_or_none()
    if existing:
        raise HTTPException(
            status_code=409,
...
async def seed_marketplace() -> None:
    ...
    async with SessionLocal() as db:
        for entry in _SEED_COMPONENTS:
            existing = (await db.execute(
                select(CommunityComponent).where(CommunityComponent.slug == entry["slug"])
            )).scalar_one_or_none()
