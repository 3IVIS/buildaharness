...
from run_api import _evict_stale_jobs, _jobs_get
...
def test_web_concurrency_guard_removed():
    ...
    assert "_jobs: dict[str, dict[str, Any]]" not in src, "run_api.py must not define the old in-memory _jobs dict."
...
async def test_evict_stale_jobs(db_engine):
    ...
    async with SessionLocal() as db:
        ...
        await db.execute(
            text(
                "INSERT INTO users (id, email, password_hash, is_active, created_at) VALUES (:id, :email, :pw, 1, :ts)"
            ),
            {
                "id": str(user_id),
    ...
    async with SessionLocal() as db:
        remaining = (
            (await db.execute(select(Job).where(Job.id.in_([old_done_id, old_error_id, fresh_id])))).scalars().all()
        )
...
async def test_a2a_duplicate_task_id_returns_409(client, auth_headers):
    ...
    r1 = await client.post(f"/a2a/{A2A_SPEC['id']}/tasks/send", json=payload, headers=auth_headers)
    assert r1.status_code == 202

    r2 = await client.post(f"/a2a/{A2A_SPEC['id']}/tasks/send", json=payload, headers=auth_headers)
