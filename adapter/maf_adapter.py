"""
buildaharness — MS Agent Framework adapter  v0.1.0
...
HITL note (Q9 resolution):
  hitl_breakpoint raises _HitlPause so the buildaharness runner can pause the job
...
def gen_harness_preamble() -> str:
    ...
    return textwrap.dedent("""\
        ...
        # ─── LangChain shims for harness node compilers ───────────────────────────
        # Harness node compilers emit code using _make_llm / _invoke_with_trace /
        # HumanMessage — originally written for the LangGraph adapter. These shims
        # make those calls work inside the MAF-generated exec() namespace.
        from langchain_openai import ChatOpenAI as _ChatOpenAI
        from langchain_core.messages import HumanMessage, SystemMessage

        def _make_llm(model: str, temperature: float = 0.7, max_tokens: int = 1024):
        ...
        def _invoke_with_trace(node_id: str, model: str, llm, messages: list):
            ...
            try:
                ...
                with _lf.start_as_current_observation(
                    name=f"llm-{node_id}", as_type="generation", model=model, input=_inp,
                ):
                    _resp = llm.invoke(messages)
                    _tok = getattr(_resp, "usage_metadata", {}) or {}
                    _lf.update_current_generation(
                        output=getattr(_resp, "content", None),
                        usage_details={"input": _tok.get("input_tokens", 0), "output": _tok.get("output_tokens", 0)},
                    )
                    return _resp
            except Exception:
                return llm.invoke(messages)
...
def gen_header(spec: dict) -> str:
    ...
    return dedent0(f"""\
        \"\"\"
        MS Agent Framework code generated by buildaharness-adapter v{ADAPTER_VERSION}
        Flow   : {name}  ({fid})
        Nodes  : {nc}  |  Edges: {ec}
...
def gen_otel_setup(spec: dict) -> str:
    ...
    project = telemetry.get("project", "buildaharness")
    ...
    return dedent0(f"""\
        ...
        def _make_kernel(model: str = {model_default!r}) -> Kernel:
            ...
            if _base_url:
                ...
                _sk_timeout = float(os.environ.get("MAF_SK_TIMEOUT", "300"))
                _client = _AsyncOpenAI(base_url=_base_url, api_key=_api_key or "ollama", timeout=_sk_timeout)
...
def gen_node_function(
    ...
    if ntype == "agent_role":
        ...
        body = (
            ...
            f"async def _collect_agent_msgs() -> None:\n"
            ...
            f"try:\n"
            f"    await asyncio.wait_for(_collect_agent_msgs(), timeout=_sk_timeout)\n"
            f"except Exception:\n"
            f"    pass  # timeout or tool error — fall through to direct LLM fallback\n"
            f"final = response_parts[-1] if response_parts else ''\n"
            f"\n"
            f"# Fallback: if the agent loop timed out, hit a tool error, or produced no text\n"
            f"# (e.g. local model only emitted tool-call frames), re-invoke directly — no tools.\n"
            f"if not final:\n"
            f"    _fb_kernel = _make_kernel({model!r})\n"
            f"    _fb_history = ChatHistory()\n"
            f"    _fb_history.add_message(ChatMessageContent(\n"
            f"        role=AuthorRole.SYSTEM, content={instructions_expr}))\n"
            f"    _fb_history.add_user_message(task)\n"
            f"    _fb_svc = _fb_kernel.get_service()\n"
            f"    _fb_settings = _fb_svc.get_prompt_execution_settings_class()()\n"
            f"    _fb_settings.temperature = 0.4\n"
            f"    _fb_settings.max_tokens = 1024\n"
            f"    try:\n"
            f"        _fb_result = await asyncio.wait_for(\n"
            f"            _fb_svc.get_chat_message_contents(_fb_history, settings=_fb_settings),\n"
            f"            timeout=_sk_timeout)\n"
            f"        final = str(_fb_result[0]) if _fb_result else ''\n"
            f"    except Exception:\n"
            f"        final = ''\n"
            f"\n"
    ...
    if ntype == "subgraph":
        ...
        body = (
            ...
            f"raise NotImplementedError({py_str(f'Subgraph {flow_ref} not wired — see comment above')})\n"
        )
        return _async_fn(label, vid, body)
    ...
    try:
        ...
        for node in sorted_nodes:
            ...
            if node["type"] == "condition":
                ...
                # Condition nodes have no async node function, but the flow runner may
                # call _exec_node for them when they appear as branch targets of other
                # conditions. Emit a pass-through stub so those calls return {} safely.
                vid = safe_id(node["id"])
                node_fns.append(
                    f"# stub for condition node used as branch target\n"
                    f"async def node_{vid}(state: dict) -> dict: return {{}}\n"
                )
