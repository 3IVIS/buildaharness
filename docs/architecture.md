...
Six further invariants were added by the harness-consolidation plan, each enforced in its own phase's test file (`test_harness_c2.py`/`nodes-c2.test.ts`, `test_harness_h.py`/`nodes-h.test.ts`, `test_harness_i.py`/`nodes-i.test.ts`, `turn-policy.test.ts`, `memory-tiers.test.ts`, `harness-runtime-suspend.test.ts`) rather than `test_harness_invariants.py`:
