...
- **INV-16** (`packages/aielia`) No Experience-tier (`procedural`) or Commitment-tier (`commitment`) entry can be read as a Knowledge belief — `TIER_RULES.procedural`/`TIER_RULES.commitment` have an empty `allowedSources`, so `tierForFact()` can structurally never route a `UserFact` into either tier.
...
These are the primitives a real per-iteration tool proposer needs, and that proposer has since landed (the internal plan, phases R1–R5): `@buildaharness/aielia`'s `AgentLoop` exposes `createHarnessProposer` / `createOneLoopProposer` / `createBatchOneLoopProposer`, each running one extracted `runToolIterationStep` per `driveMainLoop` iteration so the harness drives the real tool-calling loop directly instead of running post-hoc over an already-finished `draftReply`. Both proposers hold a real turn-scoped `TurnControlPlaneState` (`createControlPlaneState()`); the flat one also folds the harness's own live per-iteration `ControlState` in as the tool-policy gate floor, taking whichever of it and the turn-local accumulated state is more restrictive (`moreRestrictiveControlState`). It is gated by **`ASSISTANT_ONE_LOOP`** (`packages/aielia/src/one-loop-flag.ts`, resolved through the shared `AssistantConfig` seam so it reaches the CLI, `chat-ui`, and desktop identically), which **defaults to `enabled` on every surface as of 2026-09-06** (R5) — `ASSISTANT_ONE_LOOP=disabled` is the escape hatch back to the post-hoc design, byte-for-byte identical for every caller (INV-19), kept for one rollout window before the flag is removed. None of the underlying primitives change behavior for a caller on the disabled path. See `packages/harness/README.md`.
