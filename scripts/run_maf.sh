#!/usr/bin/env bash
# run_maf.sh — Submit a Microsoft Agent Framework job, handle HITL pauses, and follow to completion.
#
# Usage:
#   ./run_maf.sh flow-plan-execute.json
#
# Prompts for email + password, logs in (or auto-registers if the account
# doesn't exist yet), then runs the job end-to-end.
#
# Note: MAF resume always sends the original spec in the request body so the
# server can recompile the flow if it has restarted between pause and resume.
...
SUBMIT_RESPONSE=$(curl -s -X POST "$BASE_URL/run?runtime=microsoft_agent_framework" \
  -H "$AUTH_HEADER" \
  -H "$CONTENT_HEADER" \
