...
- **`/web/search`-specific:** a global (not per-`sub`/IP) daily ceiling on `backend=brave` calls (`WEB_BRAVE_DAILY_CEILING`), protecting the one shared `BRAVE_API_KEY` from being run up or banned by aggregate traffic.
