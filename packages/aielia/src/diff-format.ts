...
export function formatWriteDiff(oldContent: string | undefined, newContent: string, maxLines = DEFAULT_MAX_LINES): string {
  ...
  // structuredPatch (not createTwoFilesPatch's text output) so each line can carry its own
  // line number, gutter-style, the way Claude Code's own file-change display does — rather
  // than one "@@ -a,b +c,d @@" hunk header up top the reader has to do the arithmetic from.
  const hunks = structuredPatch('before', 'after', oldContent, newContent, '', '', { context: 3 }).hunks
  const entries: { lineNumber: number; sign: ' ' | '+' | '-'; content: string }[] = []
  for (const hunk of hunks) {
    let oldLine = hunk.oldStart
    let newLine = hunk.newStart
    for (const rawLine of hunk.lines) {
      // "\ No newline at end of file" is real, standard unified-diff notation (git diff/diff -u
      // emit it too) — but it's about the *file*, not the *change* being reviewed, and it reads
      // as confusing noise in a chat reply rather than a real terminal's diff pager. It isn't a
      // content line either way, so it advances neither counter.
      if (rawLine.startsWith('\\')) continue
      const sign = rawLine[0] as ' ' | '+' | '-'
  ...
  const diffLines = entries.map((e) => `${DIFF_INDENT}${String(e.lineNumber).padStart(gutterWidth, ' ')} ${e.sign}${e.content}`)
  return previewLines(diffLines, maxLines)
}
