...
describe('benchmark corpus', () => {
  ...
  it('audit_reviewer_pass slice (C6) — >= 6 review-defect stress + >= 2 clean-draft control, workspace-backed, single-turn', () => {
    const inSlice = tasks.filter((t) => t.slice === 'audit_reviewer_pass')
    const controls = inSlice.filter((t) => t.id.includes('-control-'))
    const stress = inSlice.filter((t) => !t.id.includes('-control-'))
    expect(stress.length, 'expected >= 6 stress tasks').toBeGreaterThanOrEqual(6)
    expect(controls.length, 'expected >= 2 control tasks').toBeGreaterThanOrEqual(2)
  ...
  it('audit_decomposition_multistep slice — >= 6 multi-turn stress + >= 2 multi-turn single-deliverable control', () => {
    const inSlice = tasks.filter((t) => t.slice === 'audit_decomposition_multistep')
    const controls = inSlice.filter((t) => t.id.includes('-control-'))
    const stress = inSlice.filter((t) => !t.id.includes('-control-'))
    expect(stress.length, 'expected >= 6 multi-turn stress tasks').toBeGreaterThanOrEqual(6)
    expect(controls.length, 'expected >= 2 multi-turn single-deliverable control tasks').toBeGreaterThanOrEqual(2)
    for (const t of inSlice) {
      expect(t.followups.length, `${t.id}: must be multi-turn`).toBeGreaterThanOrEqual(2)
      expect(t.workspace.length, `${t.id}: needs a fixture workspace`).toBeGreaterThanOrEqual(1)
    }
    for (const t of stress) {
      const checks = (t.grader.contains?.length ?? 0) + (t.grader.regex ? 1 : 0)
      expect(checks, `${t.id}: each tracked sub-goal needs its own check so a dropped one is a miss`).toBeGreaterThanOrEqual(3)
    }
  })
