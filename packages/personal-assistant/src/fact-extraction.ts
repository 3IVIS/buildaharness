...
// looksLikeCodingFact is a pure keyword match, so "please delete the old backup files" admits
// just as readily as "the tests passed" — the former merely mentions "files", it isn't a claim
// about the world. Excluding imperative/request phrasing keeps admission scoped to actual state
// claims; FACT_MARKERS' phrases ("my name is", "remember that", ...) are already declarative by
// construction and don't need this filter.
const IMPERATIVE_REQUEST_MARKERS = /\b(please|can you|could you|would you|will you|help me|delete|remove|run|execute|install|deploy|restart|stop|start|create|write|update|set up|change|fix|add|revert|undo)\b/i
 ...
 * message also reads as a request rather than a claim — a build/test/service-state claim like
 * "the tests passed" is exactly what the World Model's belief/contradiction machinery
...
export function extractFactsFromTurn(userMessage: string, sourceTurn: string): UserFact[] {
  ...
  const isCodingFact = looksLikeCodingFact(trimmed) && !IMPERATIVE_REQUEST_MARKERS.test(trimmed)
